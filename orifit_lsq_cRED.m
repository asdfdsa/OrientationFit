...
if failed_run < 10
    %% plot rotation track and angle btw neighbor frames for quality check
    ...
    absori=absori(:,3:5);
    sampled_absori = zeros(fn, 3);
    sum_n = round(osc_angle/0.01);
    for i = 1:fn
        if mod(sum_n,2) == 1
            sampled_absori(i, 1:3) = absori((i-1)*sum_n + (sum_n+1)/2, 1:3);
        else
            sampled_absori(i, 1:3) = (absori((i-1)*sum_n + sum_n/2, 1:3)+absori((i-1)*sum_n + sum_n/2 + 1, 1:3))/2;
        end
    end
    
    absori = sampled_absori;
